...
호스트는 임의 CSS selector 가 아니라 구조적 path 주소(`.../view/<pluginId.viewId>/node/<nodePath>`)로 이 오버레이의 요소에 접근(클릭/측정·E2E)합니다. 노출할 요소만 `data-node` 속성으로 부여하고 `plugin.json` `contributes.nodes` 에 종류를 선언합니다(동의 화면 자동 표기). 노출 안 된 요소는 접근 불가(명확한 에러).

동적 목록은 `data-node="<종류>/<안정키>"` — 안정키는 카운터 인덱스가 아니라 항목의 안정 식별자입니다(메시지는 JSONL uuid, 서브에이전트는 agent 파일 id). 안정키가 없으면 노드를 부여하지 않습니다(가짜 인덱스 금지).

| 노드 | data-node | 설명 |
|------|-----------|------|
| input | `input` | claude 입력창(textarea). `focus`/`type` 명령의 DOM 타깃. |
| send | `send` | 입력 전송 버튼(↩). |
| close | `close` | 오버레이 닫기 버튼(✕). |
| verbose | `verbose` | thinking 표시/숨김 토글(∴). |
| session | `session` | 세션 id 칩(클릭 시 전체 id 복사). |
| msg | `msg/<uuid>` | 사용자 메시지 행. 안정키 = 메시지 entry uuid. |
| agent | `agent/<id>` | 서브에이전트 패널 헤더(클릭 = 접기/펴기). 안정키 = agent 파일 id. |
